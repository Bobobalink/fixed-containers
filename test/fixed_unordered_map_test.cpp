...
TEST(FixedUnorderedMap, IteratorAssignment)
{
    constexpr auto s1 = []()
    {
...

        {
            ...
            {
                static_assert(std::is_same_v<decltype(it), FixedUnorderedMap<int, int, 10>::const_iterator>);
            }
        ...
        }
        {
            ...
            for (it = s.begin(); it != s.end(); it++)
            {
                static_assert(std::is_same_v<decltype(it), FixedUnorderedMap<int, int, 10>::iterator>);
            }
        }
        return s;
    }();
...
using InstanceCounterNonTrivialAssignment =
    instance_counter::InstanceCounterNonTrivialAssignment<FixedUnorderedMapInstanceCounterUniquenessToken>;
...
using InstanceCounterTrivialAssignment =
    instance_counter::InstanceCounterTrivialAssignment<FixedUnorderedMapInstanceCounterUniquenessToken>;
