...
// this has puts the value of an int into both the bottom 8 bits of the output and the next 8 bits of the output.
// this means that the fingerprint (the bottom 8 bits) is the same as the value, as well as the bucket index (mod the size of the table).
// this makes the following tests much more readable.
...
using IntIntMap10 = FixedRobinhoodHashtable<int, int, 10, 10, ConvenientIntHash, std::equal_to<int>>;
...
TEST(MapOperations, Emplace)
{
    ...
    // try to put another element into slot 3, but this one has a bigger key so it will push the existing two
    ...
    // now add _another_ item that goes in slot 3. We already have 3, 4, and 5 full of those, so the chunk of 3s needs to grow into 6
    // instead of jumping over 6, move 6 over because the 3 has a higher distance from its home (this is the robin hood part)
    ...
    // we already have something in slot 9, so if we try to add something to slot 8 it will bounce off the 6 and displace 9 around the end of the array
...
TEST(MapOperations, Erase)
{
    ...
    // leaving room for the 9 key (on the other side of the array) to also move. We stop bubbling after that because we encounter an empty slot
    idx = map.opaque_index_of(6);
    EXPECT_TRUE(map.exists(idx));
    EXPECT_EQ(idx.bucket_index, 8);
    out = map.erase(idx);
    ...
    // next, erase the 23 key. 13 (same hash) will shift down, allowing 66 to fall into its rightful place.
    // We then stop bubbling because the 128 key is already in the perfect place
    idx = map.opaque_index_of(23);
    EXPECT_TRUE(map.exists(idx));
    EXPECT_EQ(idx.bucket_index, 5);
    out = map.erase(idx);
...
TEST(MapOperations, LinkedListIteration)
{
    ...
    EXPECT_EQ(iter, 0);
    iter = map.advance(iter);
    EXPECT_EQ(iter, map.end_index());
    iter = map.end_index();
    iter = map.recede(iter);
    EXPECT_EQ(iter, 0);
...
TEST(MapOperations, EraseRange)
{
    ...
    // iteration is in insertion order, so erase value indices 5-7, corresponding to keys 23, 66, 128
