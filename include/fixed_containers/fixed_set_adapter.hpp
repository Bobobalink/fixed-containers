...
private:
    ...
    private:
        ...
        constexpr ReferenceProvider(const TableImpl* const table, const TableIteratedIndex& value_table_index_)
        : table_(table)
...
public:
    ...
    constexpr size_type max_size() const noexcept { return static_max_size(); }
...
public:
    ...
    explicit constexpr FixedSetAdapter(Args&&... args): IMPLEMENTATION_DETAIL_DO_NOT_USE_table_(std::forward<Args>(args)...)
    {}
...
public:
    ...
    constexpr const_iterator cend() const noexcept { return const_iterator{ReferenceProvider{&table(), table().end_index()}}; }
    ...
    {
        const ReferenceProvider& start = first.template private_reference_provider<const ReferenceProvider&>();
        const ReferenceProvider& end = last.template private_reference_provider<const ReferenceProvider&>();
    ...
    [[nodiscard]] constexpr bool operator==(const FixedSetAdapter<K, TableImpl2, CheckingType2>& other) const 
    {
        if(size() != other.size())
        {
            return false;
...
private:
    ...
    {
        return reverse_iterator{ReferenceProvider{&table(), table().iterated_index_from(start_index)}};
    }
...
template <typename K,
          ...
          typename CheckingType>
[[nodiscard]] constexpr
    typename FixedSetAdapter<K, TableImpl, CheckingType>::
        size_type
        is_full(
            const FixedSetAdapter<K, TableImpl, CheckingType>&
                c)
{
...
constexpr
    typename FixedSetAdapter<K, TableImpl, CheckingType>::
        size_type
        erase_if(
            FixedSetAdapter<K, TableImpl, CheckingType>& c,
            Predicate predicate)
{
