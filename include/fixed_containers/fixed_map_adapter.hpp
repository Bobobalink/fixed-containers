...
private:
    ...
    private:
        ...
        constexpr PairProvider(ConstOrMutableTable* const table, const TableIteratedIndex& value_table_index_)
        : table_(table)
    ...
    public:
        ...
        {
            current_index_ = table_->advance(current_index_);
        }

        constexpr void recede() noexcept
        {
            current_index_ = table_->recede(current_index_);
        }
...
public:
    [[nodiscard]] static constexpr size_type static_max_size() noexcept { return TableImpl::CAPACITY; }

public:
    TableImpl IMPLEMENTATION_DETAIL_DO_NOT_USE_table_;

private:
    constexpr TableImpl& table() {
        return IMPLEMENTATION_DETAIL_DO_NOT_USE_table_;
    }

    constexpr const TableImpl& table() const {
        return IMPLEMENTATION_DETAIL_DO_NOT_USE_table_;
    }

public:
    template <typename... Args>
    explicit constexpr FixedMapAdapter(Args&&... args): IMPLEMENTATION_DETAIL_DO_NOT_USE_table_(std::forward<Args>(args)...)
    {}
...
public:
    ...
    [[nodiscard]] constexpr const V& at(const K& key,
                                  const std_transition::source_location& loc =
    ...
    constexpr const_iterator cend() const noexcept { return const_iterator{PairProvider<true>{&table(), table().end_index()}}; }
    constexpr const_iterator begin() const noexcept { return cbegin(); }
    constexpr iterator begin() noexcept { return iterator{PairProvider<false>{&table(), table().begin_index()}}; }
    constexpr const_iterator end() const noexcept { return cend(); }
    constexpr iterator end() noexcept { return iterator{PairProvider<false>{&table(), table().end_index()}}; }

    constexpr reverse_iterator rbegin() noexcept { return reverse_iterator{PairProvider<false>{&table(), table().end_index()}}; }
    ...
    [[nodiscard]] constexpr size_type max_size() noexcept { return static_max_size(); }
    ...
    {
        return emplace_detail::emplace_in_terms_of_try_emplace_impl(*this, std::forward<Args>(args)...);
    }
    ...
    {
        const PairProvider<true>& start = first.template private_reference_provider<const PairProvider<true>&>();
        const PairProvider<true>& end = last.template private_reference_provider<const PairProvider<true>&>();
    ...
    [[nodiscard]] constexpr bool operator==(const FixedMapAdapter<K, V, MapImpl2, CheckingType2>& other) const 
    {
...
private:
    ...
    {
        // check for nonexistent indices and replace them with end() so the iterator compares correctly
    ...
    {
        return const_iterator{PairProvider<true>{&table(), table().iterated_index_from(start_index)}};
    }

    constexpr reverse_iterator create_reverse_iterator(const TableIndex& start_index) noexcept
    {
        return reverse_iterator{PairProvider<false>{&table(), table().iterated_index_from(start_index)}};
    }

    constexpr const_reverse_iterator create_const_reverse_iterator(
        const TableIndex& start_index) const noexcept
    {
        return const_reverse_iterator{PairProvider<true>{&table(), table().iterated_index_from(start_index)}};
    }
...
template <typename K,
          ...
          typename CheckingType>
[[nodiscard]] constexpr
    typename FixedMapAdapter<K, V, TableImpl, CheckingType>::
        size_type
        is_full(
            const FixedMapAdapter<K, V, TableImpl, CheckingType>&
                c)
{
...
constexpr
    typename FixedMapAdapter<K, V, TableImpl, CheckingType>::
        size_type
        erase_if(
            FixedMapAdapter<K, V, TableImpl, CheckingType>& c,
            Predicate predicate)
{
